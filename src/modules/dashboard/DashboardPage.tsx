...
export function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const unsoldItemsList = drops.filter((d) => !d.sold);
    const serviceiroDropsCount = drops.filter((d) => d.party.service).length;
    return { totalValue, soldCount, pendingCount, totalDrops: drops.length, unsoldItemsList, serviceiroDropsCount };
  }, [drops]);
    ...
    <div className="dashboard-container" style={{ padding: '20px', maxWidth: '1700px', margin: '0 auto', color: '#f8fafc' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 2.1fr 1.1fr', gap: '20px' }}>
                      ...
                      return (
                        <tr key={drop.id || idx} style={{ borderBottom: '1px solid #334155', background: rowBg }}>
                          <td style={{ padding: '6px 4px', color: '#e2e8f0', display: 'flex', alignItems: 'center', gap: '6px' }}>
                            {drop.itemIconUrl && <img src={drop.itemIconUrl} alt="" style={{ width: '18px', height: '18px', objectFit: 'contain' }} />}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          ...
          <div className="card" style={{ background: '#1e293b', padding: '15px', borderRadius: '8px', border: '1px solid #334155' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', borderBottom: '1px solid #334155', paddingBottom: '8px' }}>
              ...
              <span style={{ fontSize: '12px', background: '#0f172a', padding: '2px 6px', borderRadius: '4px' }}>
                Qtd: {stats.pendingCount}
              </span>
            </div>

            <div style={{ maxHeight: '320px', overflowY: 'auto' }}>
              {loading && <div style={{ padding: '15px', textAlign: 'center', color: '#94a3b8', fontSize: '13px' }}>Carregando...</div>}
              {error && <div style={{ padding: '15px', textAlign: 'center', color: '#ef4444', fontSize: '13px' }}>{error}</div>}
              {!loading && !error && stats.unsoldItemsList.length === 0 && (
              ...
              {!loading && !error && stats.unsoldItemsList.length > 0 && (
                <table style={{ width: '100%', fontSize: '13px', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {stats.unsoldItemsList.map((item, idx) => (
                      <tr key={item.id || idx} style={{ borderBottom: '1px solid #1e293b' }}>
                        <td style={{ padding: '6px 4px', color: '#e2e8f0' }}>{item.itemName || 'Item Raro'}</td>
                        <td style={{ padding: '6px 4px', textAlign: 'right', color: '#10b981' }}>{formatTibiaGold(item.totalValue)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="card" style={{ background: '#1e293b', padding: '15px', borderRadius: '8px', border: '1px solid #334155' }}>
            <h3 style={{ fontSize: '14px', margin: '0 0 10px 0', color: '#38bdf8', borderBottom: '1px solid #334155', paddingBottom: '8px' }}>Top Drop por Jogador</h3>
            <div style={{ fontSize: '13px', color: '#94a3b8', textAlign: 'center', padding: '20px 0' }}>
              Aguardando sincronização com a API de membros...
            </div>
          </div>
